    ...
    success: function (data) {
      if (data.status == "OK") {
        ...
        if (data.exp_supervision == 'No')
          $('#frm_seccinco_edit_exp .modal-body').find("#id_experiencia_supervision_num").attr('readonly', 'readonly');
...
$(document).ready(function () {
      ...
      if (ValidateDate(dtVal)) {
        ...
        if (fecha_mayor(dtVal)) {
          ...
          if (edad>=18){
            ...
            $("<span class='error'> Debe ser mayor de 18 años</span>").insertAfter($(this));
            //botones_f();
          }*/
        } else {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.form_is_valid) {
          ...
          $("#miModal .modal-body").html("Puedes seguir agregando o guardar sección y enviar");
          $("#miModal").modal('show');
        } else if (data.result == "passerror") {
          // $("#pass").show().fadeOut(3000);
        } else if (data.result == "user_existe") {
  ...
  $("#id_experiencia_supervision").change(function (e) {
    if ($("#id_experiencia_supervision").val() != "Si") {
      $("#id_experiencia_supervision_num").val("");
      $("#id_experiencia_supervision_num").attr("disabled", "disabled");
      $("#id_experiencia_supervision_num").attr("required", false);
    }
    else {
      $("#id_experiencia_supervision_num").removeAttr("disabled");
      $("#id_experiencia_supervision_num").attr("required", true);
      $("#id_experiencia_supervision_num").focus();
    }
  });

  $(".exp #id_experiencia_supervision").change(function (e) {

    if ($(".exp #id_experiencia_supervision").val() != "Si") {
